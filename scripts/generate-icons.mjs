...
// logo.png for brochure PDF (96x96, in src/assets for bundling)
const logoPng = await sharp(svgBuffer).resize(96, 96).png().toBuffer()
const logoOutDir = join(root, 'src', 'assets')
mkdirSync(logoOutDir, { recursive: true })
writeFileSync(join(logoOutDir, 'logo.png'), logoPng)
console.log('Generated src/assets/logo.png')
